#lang racket
(require racket/gui/base framework "../main.rkt")
...
(define (edit-history initial-data)
  ...
  (define (update-list-box)
    (define roles (map Msg-role items))
    (define contents (map (λ (h)
                            (gui-utils:quote-literal-label (Msg-content h)
                                                           #:quote-amp? #t))
                          items))
  ...
  (define (on-selection-change)
    ...
    (when (and idx (<= 0 idx (length items)))
      ...
      (send role-field set-value (Msg-role item))
      (send content-field set-value (Msg-content item))))

  (define (add-item)
    (define role (if (and (not (empty? items))
                          (string=? "user" (Msg-role (last items))))
                     "assistant"
                     "user"))
    (set! items (append items (list (make-msg role ""))))
    (update-list-box)
  ...
  (define (save-item)
    (define idx (send list-box get-selection))
    (when (and idx (<= 0 idx (sub1 (length items))))
      (define new-item (make-msg (send role-field get-value) (send content-field get-value)))
